...
## 사전 조건
- `JWT_SECRET`을 담은 Kubernetes Secret이 대상 네임스페이스에 미리 존재해야 합니다 (user-service, review-service와 공유).
  ```bash
...
helm install order-service . \
  --namespace <namespace> \
  --set image.repository=<account_id>.dkr.ecr.<region>.amazonaws.com/order-service \
  --set image.tag=<tag>
```
...
| `env.productServiceUrl` | `http://product-service` | 주문 생성 시 상품 검증에 사용하는 product-service 엔드포인트 |
