# review-service Helm Chart

## 사전 조건
- `JWT_SECRET`을 담은 Kubernetes Secret이 대상 네임스페이스에 미리 존재해야 합니다 (user-service, order-service와 공유).
  ```bash
...
- product-service가 같은 네임스페이스에 배포되어 있어야 합니다.

## 설치
```bash
helm install review-service . \
  --namespace <namespace> \
  --set image.repository=<account_id>.dkr.ecr.<region>.amazonaws.com/review-service \
  --set image.tag=<tag>
```
...
| `env.productServiceUrl` | `http://product-service` | 리뷰 작성 시 상품 검증에 사용하는 product-service 엔드포인트 |
